...
function showNotification(text) {
    try {
        ...
        newNode.className = "p-5 text-lg flex items-center border border-lic-yellow space-x-4 rounded-xl bg-lic-blue font-semibold transition-all duration-300 pointer-events-auto";
        notificationCount++;
        ...
        domAlert.classList.add('bg-black')
        domAlert.classList.remove('pointer-events-none')
        
        let opacityTimeOut = setTimeout(() => {
            newNode.classList.add("opacity-0")
            domAlert.classList.remove('bg-black')
            domAlert.classList.add('pointer-events-none')
        }, 700);
